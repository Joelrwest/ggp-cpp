...
namespace propnet
{
Role::Role(std::string_view name, std::span<const std::shared_ptr<const PropositionNode>> sees,
           std::span<const Legal> legals, std::span<const Goal> goals)
    : name{name}, sees{sees.begin(), sees.end()}, legals{legals.begin(), legals.end()},
      goals{goals.begin(), goals.end()}, role_id{allocate_role_id()}
{
}
...
{
    return std::accumulate(goals.begin(), goals.end(), 0, [this, &state](const auto accumulation, const auto &goal) {
...
std::vector<bool> Role::get_observations(const State &state) const
{
    ...
    std::transform(sees.begin(), sees.end(), std::back_inserter(observations),
                   [this, &state](const auto &see) { return see->eval(state, EMPTY_INPUTS); });
