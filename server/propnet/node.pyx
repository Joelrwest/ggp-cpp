...
cdef class Node:
    ...
    def __str__(self):
        try:
            return f'{self.__class__.__name__}(id={self.id}, node_type={self.node_type}), cons=({self.cons}), inputs=({self.inputs}), outputs=({self.outputs}), pre=({self.pre_id})'
        except AttributeError:
            pass
        try:
            return f'{self.__class__.__name__}(id={self.id}, node_type={self.node_type}), cons=({self.cons}), inputs=({self.inputs}), outputs=({self.outputs}), post=({self.post_id})'
        except AttributeError:
            pass
...
cdef class Proposition:
    ...
    def __str__(self):
        return '%s(id=%d, type=%s, gdl=%s, inputs=%s, outputs=%s)' % (
