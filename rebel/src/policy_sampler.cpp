#include "../include/policy_sampler.hpp"
#include "misc.hpp"
...
namespace rebel
{
    PolicySampler::PolicySampler(const propnet::Role& sampler_role, const propnet::Propnet& propnet) :
        propnet {propnet},
    ...
    void PolicySampler::prepare_new_game()
    {
        all_histories.clear();
    }

    void PolicySampler::add_history(const std::vector<bool>& observation, std::uint32_t prev_input)
    {
        all_histories.emplace_back(observation, prev_input);
    }

    void PolicySampler::pop_history()
    {
        all_histories.pop_back();
    }

    propnet::State PolicySampler::sample_state()
    {
    ...
    std::vector<propnet::State> PolicySampler::sample_states(std::size_t num_states)
    {
    ...
    PolicySampler::History::History(const std::vector<bool>& observation, std::uint32_t prev_input) :
    ...
    std::optional<propnet::State> PolicySampler::sample_state_impl(AllHistories::iterator all_histories_it, AllHistories::iterator all_histories_end_it, propnet::State state)
    {
        /*
    ...
    bool PolicySampler::is_invalid_state(AllHistories::iterator all_histories_it, const propnet::State& state)
    {
    ...
    void PolicySampler::add_invalid_state(AllHistories::iterator all_histories_it, const propnet::State& state)
    {
    ...
    bool PolicySampler::is_invalid_inputs(AllHistories::iterator all_histories_it, const propnet::InputSet& inputs)
    {
    ...
    void PolicySampler::add_invalid_inputs(AllHistories::iterator all_histories_it, const propnet::InputSet& inputs)
    {
