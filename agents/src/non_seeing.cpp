...
namespace agents
{
NonSeeingAgent::NonSeeingAgent(const propnet::Role &role) : SimpleAgent{role}
{
}

void NonSeeingAgent::take_observations(const propnet::State &)
{ /* Do nothing instead */
}

RandomAgent::RandomAgent(const propnet::Role &role) : NonSeeingAgent{role}
{
}
...
void FirstAgent::get_legal_inputs_impl(std::span<propnet::PropId>) const
{ /* Already in the order we want */
}

LastAgent::LastAgent(const propnet::Role &role) : NonSeeingAgent{role}
{
}
