...
namespace agents {
    ...
    std::uint32_t FirstAgent::get_legal(const std::vector<bool>& sees, std::span<const std::uint32_t> legals)
    {
        return legals.front();
    }
    ...
    std::uint32_t LastAgent::get_legal(const std::vector<bool>& sees, std::span<const std::uint32_t> legals)
    {
        return legals.back();
    }
};
